...
public class TrainControlScreen extends Screen {
    ...
    private static final Identifier TAB_SETTING_TEXTURE =
        Identifier.fromNamespaceAndPath(RealTrainModRenewed.MODID, "textures/gui/tab_setting.png");
    private static final Identifier TAB_FORMATION_TEXTURE =
    ...
    private void send(String action, int value) {
        ...
        ClientNetworkHelper.sendToServer(new TrainControlPayload(train.getId(), action, value));
        rebuildTabWidgets();
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.blit(RenderPipelines.GUI_TEXTURED, selectedTab.background, left - 1, top - 1, 0, 0, PANEL_W, PANEL_H, 256, 256);
    ...
    private void renderTabContents(GuiGraphicsExtractor graphics, int left, int top) {
        ...
        for (int i = 0; i < Math.min(6, count); i++) {
            ...
            int y = top + 30 + i * 18;
            graphics.fakeItem(new ItemStack(RealTrainModRenewedItems.TRAIN_ITEM.get()), left + 10, y - 4);
            graphics.text(font, Component.literal((i + 1) + "  " + name), left + 30, y, 0x404040, false);
        }
    }

    private void renderPlayerInventory(GuiGraphicsExtractor graphics, int left, int top) {
    ...
    private void renderInventoryItem(GuiGraphicsExtractor graphics, net.minecraft.world.entity.player.Inventory inventory, int index, int x, int y) {
    ...
    private void renderTabs(GuiGraphicsExtractor graphics, int left, int top, boolean selectedOnly) {
        for (ControlTab tab : ControlTab.values()) {
            ...
            graphics.fakeItem(tab.icon, x + 6, y + (tab.isTop ? 8 : 7));
        }
    }
    ...
    private enum ControlTab {
        SETTING("Setting", TAB_SETTING_TEXTURE, new ItemStack(RealTrainModRenewedItems.WRENCH_ITEM.get()), true),
        FUNCTION("Function", TAB_SETTING_TEXTURE, new ItemStack(RealTrainModRenewedItems.CROWBAR_ITEM.get()), true),
        FORMATION("Formation", TAB_FORMATION_TEXTURE, new ItemStack(RealTrainModRenewedItems.TRAIN_ITEM.get()), true),
        ...
        ControlTab(String label, Identifier background, ItemStack icon, boolean isTop) {
            this.label = label;
    ...
    private class DoorButton extends Button {
        ...
        protected void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
            ...
            graphics.blit(RenderPipelines.GUI_TEXTURED, TAB_INVENTORY_TEXTURE, getX() + 25, getY() + sliderOffset, 242, 80, 14, 100, 256, 256);
            graphics.blit(RenderPipelines.GUI_TEXTURED, TAB_INVENTORY_TEXTURE, getX(), getY(), 192, 0, 64, 80, 256, 256);
            graphics.blit(RenderPipelines.GUI_TEXTURED, TAB_INVENTORY_TEXTURE, getX() + 44, getY() + 48, 224, opened ? 80 : 88, 8, 8, 256, 256);
        }
    }
}
