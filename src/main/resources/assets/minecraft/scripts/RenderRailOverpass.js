...
{
	staticParts = renderer.registerParts(new Parts("base1", "base2"));
	baseL = renderer.registerParts(new Parts("baseL"));
	baseR = renderer.registerParts(new Parts("baseR"));
...
function shouldRenderObject(tileEntity, objName, len, pos)
{
	...
	{
		var modelName = tileEntity.getResourceState().getResourceSet().getConfig().getName();
		
		if(staticParts.containsName(objName) || leftParts.containsName(objName) || rightParts.containsName(objName))
		{
			return true;
		}
		else if(baseL.containsName(objName))
		{
			return modelName.contains("overpass_L") || modelName.contains("overpass_LR");
		}
		else if(baseR.containsName(objName))
		{
			return modelName.contains("overpass_R") || modelName.contains("overpass_LR");
		}
		else
		{
			return false;
 ...
 * @param move 開通時:0.0
 */
function renderRailMapDynamic(tileEntity, rms, dir, par3, move, tongIndex, isMain)
{
	var modelName = tileEntity.getResourceState().getResourceSet().getConfig().getName();
	...
	var switchType = tileEntity.getSwitch().id;
	//側壁の描画,シーサス(:2)とは片渡り(:1)メイン側のみ
	if(isMain || (switchType != 2 && switchType != 1))
	{
		if((par3 && dir == RailDir.LEFT) || (!par3 && dir == RailDir.RIGHT))
		{
			if(modelName.contains("overpass_R") || modelName.contains("overpass_LR"))
			{
				renderer.renderRailMapStatic(tileEntity, rms, max, 0, max, baseR);
			}
		}
		else
		{
			if(modelName.contains("overpass_L") || modelName.contains("overpass_LR"))
			{
				renderer.renderRailMapStatic(tileEntity, rms, max, 0, max, baseL);
			}
		}
	}
}
